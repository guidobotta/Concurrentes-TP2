...
impl EleccionLider {
    ...
    pub fn buscar_nuevo_lider(&mut self) {
        ...
        *self.id_lider.0.lock().unwrap() = None;
        ...
        if !*obtuve_ok.unwrap().0 {
            ...
            let _ = self.id_lider
                .1
                .wait_while(self.id_lider.0.lock().unwrap(), |id_lider| id_lider.is_none());
        }
    }
    ...
    fn recibir_ok(&mut self) {
        *self.obtuve_ok.0.lock().expect("[Eleccion Lider]: Error al intentar tomar el lock de ok") = true;
