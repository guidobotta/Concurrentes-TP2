...
use common::protocolo_transaccion::{ProtocoloTransaccion, MensajeTransaccion, CodigoTransaccion};
...
impl CoordinadorTransaccion {
    ...
    pub fn submit(&mut self, transaccion: &mut Transaccion) -> Resultado<()>{
        let trans_en_log = self.log.read()
    ...
    fn send_and_wait(&mut self, 
                     ...
                     esperado: MensajeTransaccion) -> Resultado<()> {
        loop {
            let responses;
            *self.responses.0
                 .lock()
                 .expect("Error al tomar lock de responses en Coordinador") = vec![None; WEBSERVICES];
        
            for (idx, mensaje) in mensajes.iter().enumerate() {
                self.protocolo.enviar(&mensaje, self.destinatarios[idx].clone())?;
            }
            responses = self.responses.1.wait_timeout_while(
                self.responses.0.lock().expect("Error al tomar lock de responses en Coordinador"), 
                TIMEOUT_WEBSERVICES, |responses| 
                    responses.iter().any(Option::is_none));
            
            match &responses {
                Ok(val) if !val.1.timed_out() => {},
                _ => {
                    println!("[COORDINADOR] Timeout de recepcion a webservices, reintentando id {}", esperado.id_op);
                    continue
                }
            };
            
            if !responses.expect("Error al tomar lock de responses en Coordinador").
                    0.iter()
    ...
    // TODO: Documentacion?? Es privada
    fn responder(mut protocolo: ProtocoloTransaccion,
                responses: Arc<(Mutex<Vec<Option<MensajeTransaccion>>>, Condvar)>,
                continuar: Arc<AtomicBool>) {
        ...
        while continuar.load(Ordering::Relaxed) {
            ...
            match mensaje.codigo {
                CodigoTransaccion::READY | CodigoTransaccion::COMMIT | CodigoTransaccion::ABORT => {
                    ...
                    responses.0.
                        lock()
                        .expect("Error al tomar lock de responses en Coordinador")[id_emisor] = Some(mensaje);
